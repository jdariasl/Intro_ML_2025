...
def Poli1():
    ...
    Z = (
        w[0] * (x1 - scaler.mean_[0]) / np.sqrt(scaler.var_[0])
        + w[1] * (x2 - scaler.mean_[1]) / np.sqrt(scaler.var_[1])
        + w[2]
    )
    ...
    ax.scatter3D(xdata, ydata, zdata)
    ax.plot_surface(
        x1, x2, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False, alpha=0.5
    )
...
def Poli2():
    ...
    Z = (
        w[0] * (x1 - scaler.mean_[0]) / np.sqrt(scaler.var_[0])
        + w[1] * (x2 - scaler.mean_[1]) / np.sqrt(scaler.var_[1])
        + w[2] * (x3 - scaler.mean_[2]) / np.sqrt(scaler.var_[2])
        + w[3] * (x4 - scaler.mean_[3]) / np.sqrt(scaler.var_[3])
        + w[4]
    )
...
def Estimator_Plot(estimator):
    ...
    scaler = StandardScaler()
    Xn = scaler.fit_transform(X)
    estimator.fit(Xn, zdata.reshape(100, 1).squeeze())
    x1 = np.linspace(np.min(xdata), np.max(xdata), num=20)
    x2 = np.linspace(np.min(ydata), np.max(ydata), num=20)
    x1, x2 = np.meshgrid(x1, x2)
    n1, n2 = x1.shape
